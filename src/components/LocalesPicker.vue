<template>
    <v-list two-line>
        <v-list-tile
            ...
            :key="option.locale"
            @click="$emit('updateCurrentLocale', option.locale)">
            <v-list-tile-content>
            ...
            <v-list-tile-action>
                <v-btn icon ripple>
                    <v-icon color="grey lighten-1">{{currentLocale === option.locale ? 'radio_button_checked' : 'radio_button_unchecked'}}</v-icon>
                </v-btn>
            </v-list-tile-action>
        </v-list-tile>
    </v-list>
